...
public List<Map<Integer,Object>> readExcel(String path,int sheetIndex, int startReadLine, int tailLine,ValidateRule[] validate) throws ValidateException, Exception {  
	...
	for(int i=startReadLine; i<sheet.getLastRowNum()-tailLine+1; i++) {  
		...
		for(Cell c : row) {  
			...
			//判断是否具有合并单元格  
			if(isMerge) {
				obj = getMergedRegionValue(sheet, row.getRowNum(), c.getColumnIndex());//获取合并后的单元格
			}else {  
				obj = getCellValue(row.getCell(c.getColumnIndex()));//获取当前的单元格
			}
			if(row.getRowNum() != startReadLine && validate != null &&  validate.length > c.getColumnIndex() && validate[c.getColumnIndex()] != null ){ 
				//第一行不校验  校验为空不校验  校验规则的长度小于列的长度不校验   校验规则为空不校验
				String res = validate[c.getColumnIndex()].validate(obj);
				if(res != null){
					throw new ValidateException(messages(row.getRowNum(),c.getColumnIndex(),res));
				}
			}
			map.put(c.getColumnIndex() ,obj); 
		}  
		list.add(map); 
	} 
